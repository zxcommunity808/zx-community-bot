...
var inlineScheduledTask = require('../inline-scheduled-task.cjs');
var ipcRunCoordinator = require('../../coordinator/ipc-run-coordinator.cjs');

async function startDaemon(message) {
    const script = await importTaskModule(message.path);
    const options = { ...(message.options || {}), logger: logger.noopLogger };
    if (options.distributed) {
        options.runCoordinator = new ipcRunCoordinator.IpcRunCoordinator(process);
    }
        ...
        switch (message.command) {
            ...
            case 'task:execute':
                try {
                    ...
                    logger.default.debug('Daemon task:execute falied:', error);
                }
                return task;
        }
    });
